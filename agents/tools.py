...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "skill",
        "description": "Invoke a registered skill by name. Skills are prompt templates loaded from .aether/skills/. Returns the skill's resolved prompt to follow.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def load_permission_rules() -> dict:
    ...
    user_settings = _load_settings(Path.home() / ".aether" / "settings.json")
    project_settings = _load_settings(Path.cwd() / ".aether" / "settings.json")
