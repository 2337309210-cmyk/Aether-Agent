...
另外支持通过 .aether/agents/*.md 定义用户自定义代理。"""
...
# 自定义代理发现结果的进程内缓存；读取 .aether/agents/*.md 后会复用，避免每次调用 agent 工具都扫目录。
...
def _discover_custom_agents() -> dict[str, dict]:
    ...
    _load_agents_from_dir(Path.home() / ".aether" / "agents", agents)
    # Project-level (higher priority, overwrites)
    _load_agents_from_dir(Path.cwd() / ".aether" / "agents", agents)
...
def reset_agent_cache() -> None:
    """清空自定义代理缓存；测试或运行中刷新 .aether/agents 配置时使用。"""
    global _cached_custom_agents
