...
配置来源：
- 全局配置：`~/.aether/settings.json`
- 项目配置：`.aether/settings.json`
...
class McpConnection:
    ...
    async def initialize(self) -> None:
        ...
        await self._send_request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "aether-agent", "version": "1.0.0"},
        })
...
class McpManager:
    ...
    def _load_configs(self) -> dict[str, dict]:
        ...
        # 1. 全局配置：~/.aether/settings.json
        global_path = Path.home() / ".aether" / "settings.json"
        self._merge_config_file(global_path, merged)

        # 2. 当前项目配置：<cwd>/.aether/settings.json
        project_path = Path.cwd() / ".aether" / "settings.json"
