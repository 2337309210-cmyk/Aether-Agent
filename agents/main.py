...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="aether-agent",
        description="Aether Agent — a minimal coding agent",
        add_help=False,
    )
...
def _resolve_api_config(cli_api_base: str | None) -> tuple[str | None, str | None, bool]:
    generic_api_key = _clean_env(os.environ.get("APIKEY")) or _clean_env(os.environ.get("AETHER_API_KEY"))
    openai_api_key = _clean_env(os.environ.get("OPENAI_API_KEY"))
    anthropic_api_key = _clean_env(os.environ.get("ANTHROPIC_API_KEY"))

    generic_api_base = _clean_env(os.environ.get("API")) or _clean_env(os.environ.get("AETHER_API_BASE"))
...
async def run_repl(agent: Agent) -> None:
    ...
    while True:
        ...
        if inp == "/skills":
            ...
            if not skills:
                print_info("No skills found. Add skills to .aether/skills/<name>/SKILL.md")
            else:
...
def main() -> None:
    ...
    if args.help:
        # 自定义帮助文本，展示 Aether Agent 支持的启动参数和 REPL 内置命令。
        print("""
Usage: aether-agent [options] [prompt]
...
Examples:
  aether-agent "fix the bug in src/app.ts"
  aether-agent --yolo "run all tests and fix failures"
  aether-agent --plan "how would you refactor this?"
  aether-agent --max-cost 0.50 --max-turns 20 "implement feature X"
  MODEL=deepseek-chat APIKEY=sk-xxx API=https://api.deepseek.com/anthropic aether-agent "hello"
  MODEL=gpt-4o OPENAI_API_KEY=sk-xxx OPENAI_BASE_URL=https://aihubmix.com/v1 aether-agent "hello"
  aether-agent --resume
  aether-agent  # starts interactive REPL
""")
        sys.exit(0)
