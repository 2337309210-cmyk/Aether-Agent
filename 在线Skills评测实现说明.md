...
本文档说明 Aether Agent 的在线 Skills 评测是怎么做的。重点解释每个阶段的名词含义、代码流程、输入输出和当前边界。
...
```text
~/.aether/skills/<skill>/SKILL.md
<project>/.aether/skills/<skill>/SKILL.md
```
...
```text
.aether/skill-evolution/online-eval/datasets/<lineage_id>/replay_pool.jsonl
```
...
```text
.aether/skill-evolution/online-eval/champions.json
.aether/skill-evolution/online-eval/champions/<lineage_id>/champion.json
.aether/skill-evolution/online-eval/champions/<lineage_id>/SKILL.md
```
...
```text
.aether/skill-evolution/online-eval/
...
```text
.aether/skill-evolution/online_eval_report.json
```
...
```text
Online skill eval:
  data_dir=/path/to/.aether/skill-evolution
